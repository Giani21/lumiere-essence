...
export default function SommelierChat() {
  ...
  // Sincronización de Sesión Real-Time
  useEffect(() => {
  ...
  // MANTENIDO: Eventos de Navbar y Scroll Lock
  useEffect(() => {
  ...
  // MEJORA: Actualización de límites proactiva
  useEffect(() => {
    if (isOpen && isAuthenticated) {
      const updateLimits = () => setRateInfo(getRateLimitInfo());
      updateLimits(); // Actualización inmediata
      const interval = setInterval(updateLimits, 60000); // Chequeo cada minuto
  ...
  const handleSend = async (e) => {
    ...
    if (!securityCheck.allowed) {
      setMessages(prev => [...prev, {
        role: 'bot',
        content: `Límite alcanzado. Podrá volver a consultar en unos minutos.`,
        recommendations: []
      }]);
    ...
    setInput('')
    setCharCount(0) // MANTENIDO: Reset de contador
    setLoading(true)
  ...
  return (
    <>
      ...
      `}>
        ...
        {/* Auth Wall MANTENIDO */}
        {!isAuthenticated && (
          <div className="absolute inset-0 z-[80] bg-white/95 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center">
            <div className="bg-white p-8 rounded-sm shadow-xl border border-gray-100 max-w-[320px] w-full flex flex-col items-center animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <a href="/login" className="w-full py-3.5 bg-primary text-accent text-[11px] font-bold uppercase tracking-widest hover:bg-primary/90 transition-colors mb-3 flex justify-center">
                Iniciar Sesión
              </a>
              <a href="/register" className="w-full py-3.5 bg-transparent border border-gray-200 text-primary text-[11px] font-bold uppercase tracking-widest hover:bg-gray-50 transition-colors flex justify-center">
                Crear Cuenta
              </a>
            </div>
            <button onClick={() => setIsOpen(false)} className="absolute top-6 right-6 p-2 bg-white rounded-full shadow-md text-gray-400 hover:text-primary transition-colors">
              <X size={20} />
            </button>
          </div>
        )}

        {/* Modal Confirmación MANTENIDO */}
        {showClearConfirm && isAuthenticated && (
          <div className="absolute inset-0 bg-black/40 z-[85] flex items-center justify-center backdrop-blur-sm transition-all rounded-sm">
            <div className="bg-white p-6 shadow-2xl max-w-[280px] w-full mx-4 border border-gray-100 text-center animate-in fade-in zoom-in duration-200">
              ...
              <p className="text-xs text-gray-500 mb-6">Se borrará el historial de esta conversación.</p>
              <div className="flex gap-2">
                <button onClick={() => setShowClearConfirm(false)} className="flex-1 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-widest bg-gray-50 hover:bg-gray-100 transition-colors">Cancelar</button>
                <button onClick={handleClearChat} className="flex-1 py-3 text-[10px] font-bold bg-primary text-accent uppercase tracking-widest hover:bg-primary/90 transition-colors">Reiniciar</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-[#FAF9F7] px-6 py-2 border-b border-gray-100 flex justify-between items-center">
          ...
          {isBlocked && isAuthenticated && <span className="text-[8px] text-red-400 uppercase font-bold tracking-tighter animate-pulse">Límite alcanzado</span>}
        </div>

        {/* Mensajes MANTENIDO con Blur dinámico */}
              ...
              {msg.recommendations?.length > 0 && (
                <div className="mt-4 w-full flex gap-4 overflow-x-auto pb-4 pl-11 snap-x snap-mandatory scrollbar-hide">
                  {msg.recommendations.map(perfume => (
                    <div key={perfume.id} className="min-w-[170px] max-w-[190px] flex-shrink-0 snap-start" onClick={() => setIsOpen(false)}>
        ...
        {/* Input Form MANTENIDO con contador */}
          ...
          <form onSubmit={handleSend} className="space-y-3">
            <div className="relative flex items-center">
              <input
                ...
                placeholder={isBlocked || !isAuthenticated ? "Consultas bloqueadas..." : "Consultar al Especialista..."}
